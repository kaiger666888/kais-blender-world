...
> 从分镜脚本到渲染参考图的完整管线调度。
> 编排 assets → layout → engine → review 四个子 skill，一键生成所有镜头的 3D 参考图。
...
│  4. 渲染执行 → 调用 layout 阶段四 + engine 执行渲染           │
...
对每个镜头执行渲染（调用 kais-blender-layout 阶段四 + engine）：

```
对每个 shot:
    1. 将蓝图转换为 layout.render_scene() 参数
    2. 根据相机要求生成多机位渲染
    3. 通过 engine API 提交渲染
    4. 收集渲染图片
```
